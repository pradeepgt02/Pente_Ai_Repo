...
async function collectFiles(testDir, respectGitIgnore) {
  ...
  const visit = async (dir, rules, status) => {
    ...
    for (const entry of entries) {
      ...
      const entryIsDirectory = entry.isDirectory() || (entry.isSymbolicLink() && import_fs2.default.statSync(entryPath).isDirectory());
...
function createClearCacheTask(config2) {
  return {
    title: "clear cache",
    setup: async () => {
      await (0, import_util14.removeDirAndLogToConsole)(import_common9.cc.cacheDir);
  ...
  return {
    ...
    setup: async (testRun, errors, softErrors) => {
      ...
      if (testRun.options.grep || testRun.options.grepInvert) {
        const grepMatcher = testRun.options.grep ? (0, import_util14.createTitleMatcher)((0, import_util14.forceRegExp)(testRun.options.grep)) : () => true;
        const grepInvertMatcher = testRun.options.grepInvert ? (0, import_util14.createTitleMatcher)((0, import_util14.forceRegExp)(testRun.options.grepInvert)) : () => false;
...
async function extractAndParseReports(dir, shardFiles, internalizer, printStatus) {
  ...
  for (const file of shardFiles) {
    ...
    printStatus(`extracting: ${(0, import_util17.relativeFilePath)(absolutePath)}`);
